...
def nearest_neighbor_classify(train_image_feats, train_labels, test_image_feats,
    ...
    if metric.lower() == 'chi2':
        ...
        actual_metric = 'cosine' # Defaulting to cosine if 'chi2' is specified, as it's safer for distances.
                                 # Or, the user could try 'chi2_kernel' if they intend to use the kernel output.
    ...
    if k == 1:
        ...
        if k <= 0:
            ...
            k = 1
        if k > train_image_feats.shape[0]: # train_image_feats.shape[0] 是训练样本数
        ...
        # Get k nearest indices for each test sample
        k_nearest_indices = np.argsort(distances, axis=1)[:, :k]
        # Get labels of these k nearest neighbors (still an array of strings)
        k_nearest_labels = train_labels_np[k_nearest_indices] # Shape: (num_test_samples, k)
        
        predicted_labels = []
        for labels_row in k_nearest_labels:
            # labels_row is a 1D array of k string labels for one test sample
            # Use collections.Counter to find the most common label
            # Counter(labels_row).most_common(1) returns a list like [('most_common_label', count)]
            if len(labels_row) > 0:
                most_common_label = Counter(labels_row).most_common(1)[0][0]
                predicted_labels.append(most_common_label)
            else:
                # Should not happen if k > 0 and k_nearest_labels is populated
                predicted_labels.append("Unknown") # Fallback for an empty row
...
def svm_classify(train_image_feats, train_labels, test_image_feats, C_value=7.0): # Added C_value parameter
    ...
    # Heuristic for dual parameter. For 'hinge' loss, dual=True is often necessary
    # unless specific conditions for dual=False are met and implemented in sklearn.
    # Defaulting to dual=True for hinge loss is safer if unsure.
    svm_params['dual'] = True # Explicitly True for hinge, or 'auto' if supported for this combination
                              # Some versions might prefer dual=False if n_samples > n_features even for hinge. Test this.
                              # For consistency with starter, let's try True. If convergence issues, dual='auto' or False (if supported for hinge) with higher max_iter can be tried.
